...
our @EXPORT    = qw/test_NoLeaks2/;    ## no critic (ProhibitAutomaticExportation)
our @EXPORT_OK = qw/NoLeaks2/;

=head1 NAME

Test::NoLeaks2 - Memory and file descriptor leak detector
...
module C<Test::NoLeaks2> implements the general idea of the approach, which
might be enough in many cases.

=head1 INTERFACE

=head3 C<< test_NoLeaks2 >>
...
C<Test::NoLeaks2> can be used to test web applications for memory leaks.
...
Please, B<do not> use C<test_NoLeaks2> more then once per test file. Consider
...
It seems a little bit strange to use C<test_NoLeaks2> or
C<NoLeaks2> in forked child, but if you really need that, please,
send PR.
...
BEGIN {
    ...
    $PAGE_SIZE = sysconf(_SC_PAGESIZE)
        or die("page size cannot be determined, Test::NoLeaks2 cannot be used");
...
sub NoLeaks2(%) {    ## no critic (ProhibitSubroutinePrototypes)
    my %args = @_;

    my ($mem, $fds, $mem_hits) = _NoLeaks2(%args);
...
sub test_NoLeaks2(%) {    ## no critic (ProhibitSubroutinePrototypes)
    my %args = @_;
    my ($mem, $fds, $mem_hits, $details) = _NoLeaks2(%args);
...
L<GitHub|https://github.com/binary-com/perl-Test-NoLeaks2>
...
L<https://github.com/binary-com/perl-Test-NoLeaks2/issues>.
